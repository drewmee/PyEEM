...
class Visualizations:
    ...
    def contour_plot(self, eem, title):
        fig, ax = plt.subplots(figsize=self.figsize)
        cf = ax.contourf(eem.columns.to_numpy(), eem.index.to_numpy(),
                         eem.to_numpy(), cmap=self.cmap)
        ...
        plt.tick_params(axis='both', which='major',
                        labelsize=self.tick_param_labelsize)
        plt.show()
    ...
    def combined_surface_contour_plot(self, eem, title):
        ...
        surf = ax.plot_surface(X, Y, Z, rstride=1, cstride=1, cmap=self.cmap,
    ...
    def single_source_animation(self, source_name, eem_df,
                                plot_type="contour"):
        fig, ax = plt.subplots(figsize=self.figsize)
        plt.xlabel(self.eem_xlabel, fontsize=self.axis_fontsize)
        plt.ylabel(self.eem_ylabel, fontsize=self.axis_fontsize)
        ...
        emission = eem_df.index.droplevel(
            ["diesel", "cigarette", "wood_smoke", "source"]).to_numpy()
        ...
        for conc in eem_df.index.get_level_values(source_name).unique():
            ...
            hmap = ax.imshow(ss, cmap=self.cmap, vmin=min_val,
                             vmax=max_val, origin='lower', extent=extent)
            camera.snap()
    ...
    def mix_animation(self, sources, mix_df):
        ...
        emission = mix_df.index.droplevel(
            ["diesel", "cigarette", "wood_smoke"]).to_numpy()
        extent = [excitation[0], excitation[-1], emission[0], emission[-1]]
        ...
        for conc_set, g in tqdm(mix_df.groupby(level=list(sources))):
            ...
            hmap = eem_ax.imshow(g, cmap=self.cmap, vmin=min_val,
                                 vmax=max_val, origin='lower', extent=extent)
