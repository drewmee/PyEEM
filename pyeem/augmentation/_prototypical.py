...
def prototypical_spectra(source_name, source_df, cal_df, hdf):
    ...
    for index, row in source_df.iterrows():
        ...
        eem_path = os.path.join(
            *["/corrections/sample_sets_raman_normalization", sample_set, sample_name]
        )
        """
        eem_path = os.path.join(*[
    ...
    if source_df[source_name].mean() != proto_concentration:
        raise Exception(
            "Mismatch between prototypical concentration "
    ...
    proto_eem = proto_eem.assign(
        **{"source": source_name, "proto_conc": proto_concentration,}
    )
    ...
    proto_eem.to_hdf(
        hdf, key=os.path.join(*["augmented", "prototypical_spectra", source_name])
    )

    return proto_eem


def single_sources(source_name, sources, c, hdf, conc_range, num_spectra):
    """[summary]
    ...
    for new_concentration in number_range:
        ...
        new_indices = sources
        ss_eem.set_index(new_indices.tolist(), append=True, inplace=True)
    ...
    aug_ss_df.to_hdf(
        hdf, key=os.path.join(*["augmented", "single_sources", source_name])
    )
