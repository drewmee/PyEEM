...
class Load:
    ...
    def data_dir(self, d):
        ...
        if not os.path.isdir(os.path.join(d, self.raw_subdir)):
            raise Exception(
                "Data directory has no subdirectory " "named raw_sample_sets"
            )
    ...
    def metadata(self, sources=None):
        ...
        try:
            ...
            raise Exception(
                "Incorrect datetime format in metadata.csv. "
        ...
        scan_types = ["water_raman", "blank_eem", "absorb", "sample_eem"]
        if not set(meta_df["scan_type"].unique().tolist()).issubset(scan_types):
            raise Exception("Unexpected scan_type types included in scan_type column")
        ...
        for name, grp in meta_df.groupby("sample_set"):
            ...
            nunique_blanks = grp[grp["scan_type"] == "blank_eem"]["scan_type"].nunique()
            if nunique_blanks != 1:
        ...
        if not set(
            ...
            raise Exception(
                "calibration_sample/prototypical_sample/"
                "test_sample/"
        ...
        ps = meta_df[meta_df["prototypical_sample"] == "y"][cols].apply(lambda x: x > 0)
        meta_df["prototypical_source"] = ps.apply(
            lambda x: np.nan
            if len(cols[x.values].values) != 1
            else cols[x.values].item(),
            axis=1,
        )

        # Create validation_sources column
        vs = meta_df[meta_df["test_sample"] == "y"][cols].apply(lambda x: x > 0)
        meta_df["test_sources"] = vs.apply(
            lambda x: np.nan
            if len(cols[x.values].values) <= 1
            else ",".join(cols[x.values]),
            axis=1,
        )
    ...
    def calibration(self, sources):
        ...
        cal_df = None
        return cal_df
        fp = os.path.join(*[self.data_dir, self.raw_subdir, "calibration.csv"])
        # There are the columns that are required for the calibration file.
        cal_df_cols = ["source", "proto_conc", "cal_func_term1"]

        # Ensure the calibration file exists where it should.
        if not os.path.isfile(fp):
            raise Exception("Calibration file does not exist at {0}".format(fp))
        ...
        if not set(cal_df_cols).issubset(cal_df.columns):
            raise Exception("Not all required columns exist in " "calibration.csv.")
    ...
    def _load_fluorolog_eem(self, data_filename):
        ...
        try:
            ...
            eem = pd.read_csv(data_filename, sep=",", skiprows=1, usecols=names)
    ...
    def _load_aqualog_abs(self, data_filename):
        absorb = pd.read_csv(
            data_filename, sep="\t", index_col=0, header=0, skiprows=[1, 2]
        )
    ...
    def _process_scan_type(self, scan_type_row):
        ...
        try:
            ...
            df.to_hdf(
                self.hdf,
                key=os.path.join(*["raw_sample_sets", str(sample_set), filename]),
            )
    ...
    def _qc_sample_set(self, sample_set):
        scan_types = ["water_raman", "blank_eem", "sample_eem", "absorb"]
        return 1
    ...
    def _process_sample_set(self, sample_set_group):
        ...
        if qc_error:
            ...
            continue
        '''
        sample_set_group.groupby(level="scan_type").apply(self._process_scan_type_group)

        return

    def sample_sets(self, raman_instrument, absorbance_instrument, eem_instrument):
        """[summary]
        ...
        if raman_instrument is not None:
            self.raman_instrument = (
                _supported[_supported["name"] == raman_instrument]["object"]
                .unique()
                .item()
            )

        self.absorbance_instrument = (
        ...
        self.meta_df.groupby(level="sample_set").apply(self._process_sample_set)
...
def load_rutherford(demo_data_dir):
    ...
    hdf = load.sample_sets(
        raman_instrument=None, absorbance_instrument="aqualog", eem_instrument="aqualog"
    )
